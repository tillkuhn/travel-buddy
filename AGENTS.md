...
- The optional remote AI gateway (`GatewayProxyController` + OAuth2 client-credentials) is disabled by default and gitignored (`application-local.properties`). It's gated by `@ConditionalOnProperty("gateway.auth.token-url")` — don't assume it's active unless that property is set.
