...
- The optional remote AI gateway (OAuth2 client-credentials, wired via `GatewayAuthConfig` using standard Spring Security OAuth2 client machinery — no local proxy) is disabled by default and gitignored (`application-local.properties`). It's gated by `@ConditionalOnProperty("gateway.auth.token-url")` — don't assume it's active unless that property is set.
